...
resource "coder_script" "install-gh-cli" {
  ...
  script = templatefile("${path.module}/scripts/install-gh-cli.sh.tftpl", {
    version = "2.100.0" # renovate: datasource=github-releases depName=cli/cli extractVersion=^v(?<version>.+)$
  })
  run_on_start       = true
...
module "mux" {
  ...
  version              = "1.5.0"
  install_version      = "0.28.5" # renovate: datasource=github-releases depName=coder/mux extractVersion=^v(?<version>.+)$
  agent_id             = coder_agent.main.id
