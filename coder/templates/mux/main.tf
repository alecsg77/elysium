...
resource "coder_script" "install-gh-cli" {
  ...
  script = templatefile("${path.module}/scripts/install-gh-cli.sh.tftpl", {
    version = "2.101.0" # renovate: datasource=github-releases depName=cli/cli extractVersion=^v(?<version>.+)$
  })
  run_on_start       = true
  start_blocks_login = false
}

resource "coder_script" "install_docker_diagnostics" {
  count              = data.coder_workspace.me.start_count
  agent_id           = coder_agent.main.id
  display_name       = "Install Docker diagnostics"
  icon               = "/icon/terminal.svg"
  script             = file("${path.module}/scripts/install-docker-diagnostics.sh")
  run_on_start       = true
  start_blocks_login = false
}
...
module "mux" {
  ...
  version              = "2.0.0"
  install_version      = "0.29.0" # renovate: datasource=github-releases depName=coder/mux extractVersion=^v(?<version>.+)$
  agent_id             = coder_agent.main.id
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name              = "dev"
          image             = "ghcr.io/coder/envbox:0.6.9"
          ...
          env {
            ...
            value = "codercom/enterprise-node:ubuntu-20260914"
          }
          env {
