...
Use break-glass only when `PR Validate Simple / validate` is itself defective or
unavailable and blocks the repair PR. It is not a response to a failed application
deployment, slow reconciliation, or inconvenient validation result.
...
2. In GitHub repository Rules, remove **only** `PR Validate Simple / validate` from
   the `Protection` ruleset. Keep pull-request-only, deletion, and non-fast-forward
   rules active and do not add a bypass actor.
3. Open a PR that repairs the gate. Run every applicable validator locally and
   attach exact commands/results to the PR.
4. Enable native squash auto-merge. Do not use `--admin` and do not push directly
   to `main`.
5. Immediately restore `PR Validate Simple / validate` after the repair reaches
   `main`.
...
- The ruleset again requires PRs and `PR Validate Simple / validate`.
- The bypass list is unchanged.
- A test PR runs the check from the current default branch.
